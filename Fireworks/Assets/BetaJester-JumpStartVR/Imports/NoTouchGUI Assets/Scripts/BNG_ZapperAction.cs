...
#if UNITY_EDITOR
    [Tooltip("If this is set to true, the button will activate the item and reset immediately.")]
#endif
    public bool resetImmediately = false; //if set to true, it will only activate the item and no deactivation will ever take place, useful for continue and quit buttons that appear and disappear but have no deactivation state
...
#if UNITY_EDITOR
    [Tooltip("A gameobject that will be sent to your game object with the function")]
#endif
    public GameObject OnActivateValue;
    ...
    IEnumerator WaitToActivate(float waitTime) //this handles what happens on Activationthe cooldown period
    {
        ...
        if (!isActivated && CallFunctionOnActivate != "") //if it's not activated and you have a function to call, do it
        {
            if (GameObjectToActivate != null)
            {
                if (oneTimeUse)
                {
                    if (!debugSendMessage) GameObjectToActivate.SendMessage(CallFunctionOnActivate.ToString(), OnActivateValue, SendMessageOptions.DontRequireReceiver);
                    else GameObjectToActivate.SendMessage(CallFunctionOnActivate.ToString(), OnActivateValue, SendMessageOptions.RequireReceiver);
        ...
        else
        {
            ...
            else if (transform.gameObject.activeInHierarchy) //object has been activated and is still visible on screen
            {
