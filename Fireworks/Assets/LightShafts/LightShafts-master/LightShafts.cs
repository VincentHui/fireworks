...
public partial class LightShafts : MonoBehaviour
{
    ...
    void Update()
    {
        if (m_Cameras.Length < 2)
        {
            m_Cameras = new Camera[0];

            GameObject Left = GameObject.Find("CameraLeft");
            GameObject Right = GameObject.Find("CameraRight");

            if (Left)
            {
                m_Cameras = new Camera[] { Left.GetComponent<Camera>(), Right.GetComponent<Camera>() };
            }
        }
    }
