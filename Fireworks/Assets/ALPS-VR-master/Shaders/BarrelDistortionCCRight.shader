Shader "Custom/BarrelDistortionCCRight" {
	...
	float4 frag(v2f i) : SV_Target 
	{
        ...
        float2 ccRadialOffset  = radialOffset * _cc;
        float2 radialOffsetIPD = radialOffset + _CenterIPD;
		float4 green = tex2D(_MainTex ,radialOffsetIPD);
		float4 red = tex2D(_MainTex , radialOffsetIPD-ccRadialOffset);
		float4 blue = tex2D(_MainTex, radialOffsetIPD+ccRadialOffset);
		float4 colorcc = float4(red.r,green.g,blue.b,1.0f);
